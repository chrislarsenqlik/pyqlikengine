...
class EngineAppApi:
    ...
    # GetScript method: Return the full text of the load script
    ...
    def add_alternate_state(self, doc_handle, state_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetScript", "params": [state_name]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # AddFieldFromExpression method: Adds a field on the fly. !! The expression of a field on the fly is persisted but not its values. !!
    def add_field_from_expression(self, doc_handle, field_name, expr_value):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "AddFieldFromExpression", "params": [field_name, expr_value]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    def check_expression(self, doc_handle, expr_value):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "CheckExpression", "params": [expr_value]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    def check_script(self, doc_handle, expr_value):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "CheckScriptSyntax", "params": [expr_value]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    def clear_all(self, doc_handle, locked_also=false, alt_state=""):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "ClearAll", "params": [locked_also, alt_state]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # CreateConnection method: Creates a connection. A connection indicates from which data source, the data should be taken.
    # The connection can be: an ODBC connection, OLEDB connection, a custom connection, a folder connection (lib connection), an internet connection, Single Sign-On
    def create_connection(self, doc_handle, connect_name, connect_string, connect_type, user_name, password, mod_date="", meta="", sso_passthrough="LOG_ON_SERVICE_USER"):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "CreateConnection", "params": [{
          "qName": connect_name,
          "qMeta": meta,
    ...
    def create_master_dim(self, doc_handle, dim_id, dim_title, dim_grouping="N", dim_field, dim_label, meta_def=""):
    ...
    def destroy_dim(self, doc_handle, dim_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroyDimension", "params": [{dim_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # DestroyMeasure method: Removes a measure
    def destroy_measure(self, doc_handle, measure_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroyDimension", "params": [{measure_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    def destroy_object(self, doc_handle, object_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroyObject", "params": [{object_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    def destroy_session_object(self, doc_handle, object_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroySessionObject", "params": [{object_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # DestroySessionVariable method: Removes an transient variable.
    def destroy_session_variable(self, doc_handle, var_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroySessionVariable", "params": [{var_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # DestroyVariableById method: Removes a varable.. Script-defined variables cannot be removed using the DestroyVariableById method or the DestroyVariableByName method. 
    def destroy_variable_by_id(self, doc_handle, var_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroyVariableById", "params": [{var_name}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']

    # DestroyVariableByName method: Removes a varable.. Script-defined variables cannot be removed using the DestroyVariableById method or the DestroyVariableByName method. 
    def destroy_variable_by_id(self, doc_handle, var_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DestroyVariableByName", "params": [{var_id}]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    # CreateObject method: Creates a generic object at app level.  It is possible to create a generic object that is linked to another object. A linked object is an object that points to a linking object. The linking object is defined in the properties of the linked object (in qExtendsId). The linked object has the same properties as the linking object.
    # TODO: Come back to this - Very important that it is well understood how we want to create objects / datasets from python in app
    ...
    def create_session_variable(self, doc_handle, var_id="", var_name, var_comment="", var_def):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "CreateSessionVariable", "params": [{
          "qInfo": {
            "qId": var_id,
            "qType": "Variable"
          },
    ...
    def do_reload(self, doc_handle, reload_mode=0, partial_mode=false, debug_mode=false):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "DoReload", "params": [reload_mode,partial_mode,debug_mode]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']
    ...
    def expr_eval(self, doc_handle, expr):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "EvaluateEx", "params": {"qExpression": expr}})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_connection(self, doc_handle, connection_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetConnection", "params": [connection_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_db_info(self, doc_handle, connection_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabaseInfo", "params": [connection_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_db_owners(self, doc_handle, connection_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabaseOwners", "params": [connection_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_databases(self, doc_handle, connection_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabases", "params": [connection_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']['qOwners']
    ...
    def get_db_table_fields(self, doc_handle, connection_id, db_name="", db_owner="", table_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabaseTableFields", "params": [connection_id, db_name, db_owner, table_name]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_db_table_preview(self, doc_handle, connection_id, db_name="", db_owner="", table_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabaseTablePreview", "params": [connection_id, db_name, db_owner, table_name]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_db_tables(self, doc_handle, connection_id, db_name="", db_owner=""):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDatabaseTables", "params": [connection_id, db_name, db_owner]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']['qPreview']
    ...
    def get_dim_handle(self, doc_handle, dim_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetDimension", "params": [dim_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_field_descr(self, doc_handle, field_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetFieldDescription", "params": {"qFieldName": field_name}})
        response = self.engine_socket.send_call(self.engine_socket, msg)
    ...
    def get_field_handle(self, doc_handle, field_name):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetField", "params": [field_name]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
        return json.loads(response)['result']['qReturn']
    ...
    def get_file_table_fields(self, doc_handle, connection_id, rel_path="", data_fmt="", table_name=""):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetFileTableFields", "params": [
            connection_id,
            rel_path,
            {
              "qType": data_fmt
    ...
    def get_folder_items_for_connection(self, doc_handle, connection_id):
        msg = json.dumps({"jsonrpc": "2.0", "id": 0, "handle": doc_handle, "method": "GetFolderItemsForConnection", "params": [connection_id]})
        response = self.engine_socket.send_call(self.engine_socket, msg)
